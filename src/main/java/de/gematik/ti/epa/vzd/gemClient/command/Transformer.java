  ...
  public static CreateDirectoryEntry getCreateDirectoryEntry(CommandType command) {
    ...
    if (command.getUserCertificate() != null) {
      createDirectoryEntry
          .setUserCertificates(getUserCertificates(command.getUserCertificate()));
    }
    return createDirectoryEntry;
  }

  private static List<UserCertificate> getUserCertificates(
      UserCertificateType userCertificateType) {
    List<UserCertificate> userCertificateList = new ArrayList<>();
    ...
    if (StringUtils.isNoneBlank(userCertificateType.getUserCertificate())) {
      userCertificate
          .setUserCertificate(userCertificateType.getUserCertificate().replaceAll("\\n", ""));
    }
