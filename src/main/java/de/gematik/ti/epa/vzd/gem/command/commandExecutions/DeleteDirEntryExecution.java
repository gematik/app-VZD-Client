...
public class DeleteDirEntryExecution extends ExecutionBase {
    ...
    protected ExecutionResult executeCommand(CommandType command, GemApiClient apiClient) throws ApiException {
        ...
        if (response.getStatusCode() == HttpStatus.SC_OK) {
            return new ExecutionResult("Delete directory entry execution successful operated for " + command.getDn().getUid(), true,
                response.getStatusCode());
        } else if (response.getStatusCode() == HttpStatus.SC_NOT_FOUND) {
            return new ExecutionResult(command.getDn().getUid() + " could not be found", true, response.getStatusCode());
        } else {
