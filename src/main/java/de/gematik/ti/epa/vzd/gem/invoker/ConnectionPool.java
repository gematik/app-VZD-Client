...
public class ConnectionPool implements IConnectionPool {
    ...
    public synchronized GemApiClient getConnection() throws InterruptedException {
        LOG.trace("Try to get Connection (" + getUsedConnectionSize() + " / " + getConnectionSize() + ")");
        GemApiClient connection = connections.take();
        usedConnections.add(connection);
        LOG.trace("Connection found and return (" + getUsedConnectionSize() + " / " + getConnectionSize() + ")");
        return connection;
    }

    @Override
    public synchronized void releaseConnection(GemApiClient gemApiClient) {
        LOG.trace("Try to releaseConnection (" + getUsedConnectionSize() + " / " + getConnectionSize() + ")");
        if (usedConnections.contains(gemApiClient)) {
            usedConnections.remove(gemApiClient);
            connections.add(gemApiClient);
            LOG.trace("Connection released (" + getUsedConnectionSize() + " / " + getConnectionSize() + ")");
    ...
    public void reset() {
        ...
        LOG.trace("Connection reset done (" + getUsedConnectionSize() + " / " + getConnectionSize() + ")");
    }
