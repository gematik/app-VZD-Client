...
public class AddDirEntryExecution extends ExecutionBase {
    ...
    public boolean checkValidation(CommandType command) {
        ...
        if (command.getUserCertificate().isEmpty()) {
            check = StringUtils.isNotBlank(command.getTelematikID());
        } else {
    ...
    private ExecutionResult doModify(CommandType command, GemApiClient apiClient) throws ApiException {
        ...
        ExecutionResult modExecutionResult = ExecutionCollection.getInstance().getModifyDirEntry().executeCommand(command, apiClient);
        sb.append(modExecutionResult.getMessage() + "\n +++ proceeded as modify +++");
        return new ExecutionResult(sb.toString(), modExecutionResult.getResult(), modExecutionResult.getHttpStatusCode());
    }
