...
public class ReadDirEntryCertExecution extends ExecutionBase {
    ...
    protected ExecutionResult executeCommand(CommandType command, GemApiClient apiClient)
        ...
        if (result.getData().size() == 0) {
            return new ExecutionResult("No certificate could be found for " + Transformer.getBaseDirectoryEntryFromCommandType(command), false,
                404);
        }
        StringBuffer sb = new StringBuffer();
        result.getData().forEach(directoryEntry -> sb.append("Entry found: " + directoryEntry));
        return new ExecutionResult(sb.toString(), true, result.getStatusCode());
    }

    public ApiResponse<List<UserCertificate>> getResult(CommandType command, GemApiClient apiClient) throws ApiException {
